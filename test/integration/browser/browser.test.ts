...
import type {default as maplibregpu, Map} from '../../../dist/maplibre-gl';
...
let page: Page;
let map: Map;
let maplibregpu: typeof maplibregpu;
...
describe('Browser tests', () => {
    ...
    test('Load should fire before resize and moveend', async () => {
        const firstFiredEvent = await page.evaluate(() => {
            const map2 = new maplibregpu.Map({
                container: 'map',
    ...
    test('Drag to the left', async () => {
        const canvas = await page.$('.maplibregpu-canvas');
    ...
    test('Marker scaled: correct drag', async () => {
        await page.evaluate(() => {
            ...
            (window as any).marker = new maplibregpu.Marker({draggable: true})
        ...
        const fullscreenButtonTitle = await page.evaluate(async () => {
            ...
            class MapLibre extends HTMLElement {
                async connectedCallback() {
                    ...
                    map.addControl(new maplibregpu.FullscreenControl());
                }
            }
            ...
            await map.once('idle');
            const fullscreenButton = document.getElementsByTagName('map-libre')[0].shadowRoot.querySelector('.maplibregpu-ctrl-fullscreen') as HTMLButtonElement;
    ...
    test('Load map with RTL plugin should throw exception for invalid URL', async () => {
        ...
        const rtlPromise = page.evaluate(() => {
            ...
            return maplibregpu.setRTLTextPlugin('badURL', false);
        });
