...
import type {Map, CanvasSource, PointLike, StyleSpecification} from '../../../dist/maplibre-gpu';
import jnuitReportBuilder, {type TestSuite} from 'junit-report-builder';
import * as maplibreglModule from '../../../dist/maplibre-gpu';

const __dirname = dirname(fileURLToPath(import.meta.url));
let maplibregpu: typeof maplibreglModule;
...
<body>
    ...
    const evaluatedArray = await page.evaluate(async (style: StyleWithTestData) => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (maplibregpu.getRTLTextPluginStatus() === 'unavailable') {
                maplibregpu.setRTLTextPlugin(
            ...
            const engine = map.painter.engine;
            ...
            const w = options.reportWidth ?? engine._renderingCanvas.width;
            const h = options.reportHeight ?? engine._renderingCanvas.height;
