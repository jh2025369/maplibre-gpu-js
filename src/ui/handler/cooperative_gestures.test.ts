...
describe('CoopGesturesHandler', () => {
    ...
    test('Does not zoom on wheel if no key is down', async () => {
        ...
        map._renderTaskQueue.run();
        expect(map.getContainer().querySelector('.maplibregpu-cooperative-gesture-screen.maplibregpu-show')).toBeInstanceOf(HTMLDivElement);
        ...
        expect(map.getContainer().querySelector('.maplibregpu-cooperative-gesture-screen.maplibregpu-show')).toBeNull();
        map.remove();
    });
