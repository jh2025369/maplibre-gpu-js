...
export class Map extends Camera {
    ...
    addControl(control: IControl, position?: ControlPosition): Map {
        ...
        this._controlsPosition.push(position);
    ...
    initControls() {
        for (const i in this._controls) {
            ...
            const controlElement = control.onAdd(this);
            const position = this._controlsPosition[i];
            const positionContainer = this._controlPositions[position];
            if (position.indexOf('bottom') !== -1) {
                positionContainer.insertBefore(controlElement, positionContainer.firstChild);
            } else {
    ...
    _setupContainer() {
        ...
        container.classList.add('maplibregpu-map');

        const canvasContainer = this._canvasContainer = DOM.create('div', 'maplibregpu-canvas-container', container);
        if (this._interactive) {
            canvasContainer.classList.add('maplibregpu-interactive');
        }

        this._canvas = DOM.create('canvas', 'maplibregpu-canvas', canvasContainer);
        ...
        const controlContainer = this._controlContainer = DOM.create('div', 'maplibregpu-control-container', container);
        const positions = this._controlPositions = {};
        ['top-left', 'top-right', 'bottom-left', 'bottom-right'].forEach((positionName) => {
            positions[positionName] = DOM.create('div', `maplibregpu-ctrl-${positionName} `, controlContainer);
        });
    ...
    async _setupPainter() {
        ...
        this.painter = new Painter(engine, this.transform);
    }
    ...
    remove() {
        ...
        this._container.classList.remove('maplibregpu-map');
