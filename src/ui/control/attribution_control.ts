...
export class AttributionControl implements IControl {
    ...
    onAdd(map: Map) {
        ...
        this._container = DOM.create('details', 'maplibregpu-ctrl maplibregpu-ctrl-attrib');
        this._compactButton = DOM.create('summary', 'maplibregpu-ctrl-attrib-button', this._container);
        this._compactButton.addEventListener('click', this._toggleAttribution);
        this._setElementTitle(this._compactButton, 'ToggleAttribution');
        this._innerContainer = DOM.create('div', 'maplibregpu-ctrl-attrib-inner', this._container);
    ...
    _toggleAttribution = () => {
        if (this._container.classList.contains('maplibregpu-compact')) {
            if (this._container.classList.contains('maplibregpu-compact-show')) {
                this._container.setAttribute('open', '');
                this._container.classList.remove('maplibregpu-compact-show');
            } else {
                this._container.classList.add('maplibregpu-compact-show');
    ...
    _updateAttributions() {
        ...
        if (attributions.length) {
            ...
            this._container.classList.remove('maplibregpu-attrib-empty');
        } else {
            this._container.classList.add('maplibregpu-attrib-empty');
        }
        this._updateCompact();
    ...
    _updateCompact = () => {
        if (this._map.getCanvasContainer().offsetWidth <= 640 || this._compact) {
            if (this._compact === false) {
                this._container.setAttribute('open', '');
            } else if (!this._container.classList.contains('maplibregpu-compact') && !this._container.classList.contains('maplibregpu-attrib-empty')) {
                this._container.setAttribute('open', '');
                this._container.classList.add('maplibregpu-compact', 'maplibregpu-compact-show');
            }
        } else {
            this._container.setAttribute('open', '');
            if (this._container.classList.contains('maplibregpu-compact')) {
                this._container.classList.remove('maplibregpu-compact', 'maplibregpu-compact-show');
            }
        }
    };
