...
export class LogoControl implements IControl {
    ...
    onAdd(map: Map) {
        ...
        this._container = DOM.create('div', 'maplibregpu-ctrl');
        const anchor = DOM.create('a', 'maplibregpu-ctrl-logo');
        ...
        if (containerChildren.length) {
            ...
            if (this._map.getCanvasContainer().offsetWidth <= 640 || this._compact) {
                if (this._compact !== false) {
                    anchor.classList.add('maplibregpu-compact');
                }
            } else {
                anchor.classList.remove('maplibregpu-compact');
            }
        }
    };
