 ...
 * ```
 */
export class ImgSource extends Evented implements Source {
    type: string;
    id: string;
    minzoom: number;
    maxzoom: number;
    ...
    setCoordinates(coordinates: Coordinates): this {
        ...
        if (this.boundsBuffer) {
            this.boundsBuffer.underlyingResource.destroy();
    ...
    prepare() {
        ...
        const engine = this.map.painter.engine;

        if (!this.boundsBuffer) {
            this.boundsBuffer = engine.createVertexBuffer(this._boundsArray.arrayBuffer);
        }
