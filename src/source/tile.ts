...
export class Tile {
    ...
    unloadVectorData() {
        ...
        if (this.imageAtlasTexture) {
            this.imageAtlasTexture.releaseInternalTexture();
        }
        ...
        if (this.glyphAtlasTexture) {
            this.glyphAtlasTexture.releaseInternalTexture();
        }
    ...
    upload(engine: WebGPUEngine) {
        for (const id in this.buckets) {
            ...
            if (bucket.uploadPending()) {
                bucket.upload(engine);
            }
            bucket.engine = engine;
        }

        if (this.imageAtlas && !this.imageAtlas.uploaded) {
            this.imageAtlasTexture = engine.createTextureNoUrl(
            ...
            this.imageAtlasTexture.wrapU = Constants.TEXTURE_CLAMP_ADDRESSMODE;
            this.imageAtlasTexture.wrapV = Constants.TEXTURE_CLAMP_ADDRESSMODE;
            this.imageAtlas.uploaded = true;
        }

        if (this.glyphAtlasImage) {
            this.glyphAtlasTexture = engine.createTextureNoUrl(
                this.glyphAtlasImage,
                true,
                false,
                false,
                Texture.BILINEAR_SAMPLINGMODE,
                this.glyphAtlasImage.data.buffer,
    ...
    prepare(imageManager: ImageManager, engine: WebGPUEngine) {
        if (this.imageAtlas) {
            this.imageAtlas.patchUpdatedImages(imageManager, this.imageAtlasTexture, engine);
        }
    }
