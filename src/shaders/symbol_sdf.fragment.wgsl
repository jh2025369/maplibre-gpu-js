...
fn main(input: FragmentInput) -> @location(0) vec4f {
    var EDGE_GAMMA: f32 = 0.105 / uniforms.u_device_pixel_ratio;
    ...
    var inner_edge: f32 = (256.0 - 64.0) / 256.0;
    ...
    var alpha: f32 = smoothstep(inner_edge - gamma_scaled, inner_edge + gamma_scaled, dist);
    if (uniforms.u_is_halo == 1) {
        // When drawing halos, we want the inside of the halo to be transparent as well
        // in case the text fill is transparent.
        var halo_edge: f32 = (6.0 - input.v_halo_width / fontScale) / SDF_PX;
