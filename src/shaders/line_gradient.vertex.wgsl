...
struct VertexInput {
    ...
    @location(2) a_uv_x: f32,
    @location(3) a_split_index: f32,
    @location(4) a_blur: f32,
    @location(5) a_opacity: f32,
    @location(6) a_gapwidth: f32,
    @location(7) a_offset: f32,
    @location(8) a_width: f32,
};
...
fn main(input: VertexInput) -> VertexOutput {
    ...
    var texel_height: f32 = 1.0 / uniforms.u_image_height;
    var half_texel_height: f32 = 0.5 * texel_height;
    output.v_uv = vec2f(input.a_uv_x, input.a_split_index * texel_height - half_texel_height);
    ...
    output.v_width2 = vec2(outset, inset);
