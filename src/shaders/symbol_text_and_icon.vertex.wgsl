...
fn main(input: VertexInput) -> VertexOutput {
    ...
    var is_sdf: f32 = a_size[0] - 2.0 * a_size_min;
    ...
    if (uniforms.u_rotate_symbol == 1) {
        ...
        var offsetProjectedPoint: vec4f = u_matrix * vec4f(a_pos + vec2f(1, 0), ele, 1);
    ...
    output.position = uniforms.u_coord_matrix * vec4f(projected_pos.xy / projected_pos.w + 
        rotation_matrix * (a_offset / 32.0 * fontScale), z, 1.0);
    ...
    output.v_data0.xy = a_tex / uniforms.u_texsize;
    output.v_data0.zw = a_tex / uniforms.u_texsize_icon;
    output.v_data1 = vec4f(gamma_scale, size, interpolated_fade_opacity, is_sdf);
