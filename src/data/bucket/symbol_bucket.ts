...
export class SymbolBuffers {
    ...
    upload(engine: WebGPUEngine, dynamicIndexBuffer: boolean, upload?: boolean, update?: boolean) {
        if (this.isEmpty()) {
            return;
        }
    ...
    destroy() {
        ...
        this.dynamicLayoutVertexBuffer.underlyingResource.destroy();
        this.opacityVertexBuffer.underlyingResource.destroy();
    }
}
...
class CollisionBuffers {
    ...
    upload(engine: WebGPUEngine) {
        this.layoutVertexBuffer = engine.createVertexBuffer(this.layoutVertexArray.arrayBuffer);
        this.indexBuffer = engine.createIndexBuffer(this.indexArray.arrayBuffer);
        this.collisionVertexBuffer = engine.createVertexBuffer(this.collisionVertexArray.arrayBuffer);
    }
    ...
    destroy() {
        ...
        this.indexBuffer.underlyingResource.destroy();
        this.segments.destroy();
        this.collisionVertexBuffer.underlyingResource.destroy();
    }
}
...
export class SymbolBucket implements Bucket {
    ...
    upload(engine: WebGPUEngine) {
        if (!this.uploaded && this.hasDebugData()) {
            this.textCollisionBox.upload(engine);
            this.iconCollisionBox.upload(engine);
        }
        this.text.upload(engine, this.sortFeaturesByY, !this.uploaded, this.text.programConfigurations.needsUpload);
        this.icon.upload(engine, this.sortFeaturesByY, !this.uploaded, this.icon.programConfigurations.needsUpload);
        this.uploaded = true;
    }
    ...
    sortFeatures(angle: number) {
        ...
        if (this.text.indexBuffer) this.engine.updateDynamicVertexBuffer(this.text.indexBuffer, this.text.indexArray.arrayBuffer);
        if (this.icon.indexBuffer) this.engine.updateDynamicVertexBuffer(this.icon.indexBuffer, this.icon.indexArray.arrayBuffer);
    }
}
