...
import { ITextureCreationOptions, Texture } from "../Materials/Textures/texture";
...
export class WebGPUEngine extends ThinWebGPUEngine {
    ...
    private _viewportsDepthRangeCurrent: { minDepth: number; maxDepth: number } = { minDepth: 0, maxDepth: 1 };
    ...
    private _mustUpdateViewportDepth(): boolean {
        const minDepth = this._viewportDepthRangeCached.minDepth,
            maxDepth = this._viewportDepthRangeCached.maxDepth;

        const update = this._viewportsDepthRangeCurrent.minDepth !== minDepth || this._viewportsDepthRangeCurrent.maxDepth !== maxDepth;

        if (update) {
            this._viewportsDepthRangeCurrent.minDepth = this._viewportDepthRangeCached.minDepth;
            this._viewportsDepthRangeCurrent.maxDepth = this._viewportDepthRangeCached.maxDepth;
        }
    ...
    private _applyViewport(bundleList: Nullable<WebGPUBundleList>): void {
        ...
        const minDepth = this._viewportDepthRangeCached.minDepth;
        const maxDepth = this._viewportDepthRangeCached.maxDepth;
        ...
        if (bundleList) {
            ...
            this._getCurrentRenderPass().setViewport(x, y, w, h, minDepth, maxDepth);
        }
    ...
    public _viewportDepthRange(minDepth: number, maxDepth: number): void {
        this._viewportDepthRangeCached.minDepth = minDepth;
        this._viewportDepthRangeCached.maxDepth = maxDepth;
    }
     ...
     */
    public createIndexBuffer(indices: IndicesArray | ArrayBuffer, _updatable?: boolean, label?: string): DataBuffer {
        let is32Bits = true;
     ...
     * @param name The name of the uniform in the effect
     */
    public setTexture2(texture: Nullable<BaseTexture>, name: string): void {
        this._setTexture(-1, texture, false, false, name, name);
    }
    ...
    public _startMainRenderPass(setClearStates: boolean, clearColor?: Nullable<IColor4Like>, clearDepth?: boolean, clearStencil?: boolean): void {
        this._endCurrentRenderPass();
    ...
    private _applyRenderPassChanges(bundleList: Nullable<WebGPUBundleList>): void {
        ...
        if (this._mustUpdateViewport() || this._mustUpdateViewportDepth()) {
    ...
    public _draw(drawType: number, fillMode: number, start: number, count: number, instancesCount: number): void {
    ...
    //                              extensions
    //------------------------------------------------------------------------------
    public createTextureNoUrl(
        size: TextureSize,
        noMipmapOrOptions: boolean | ITextureCreationOptions,
        invertY: boolean,
        premultiplyAlpha: boolean,
        samplingMode: number,
        buffer: ArrayBuffer | ImageBitmap | HTMLImageElement | ArrayBufferLike,
        format: number
    ): Texture {
        const texture = new Texture("", this, noMipmapOrOptions, invertY, samplingMode, null, null, buffer, false, format, undefined, undefined, 0, "");
        ...
        const internalTexture = this._createInternalTexture(
            size,
            {
                generateMipMaps: !noMipmap,
                samplingMode,
                format,
                useSRGBBuffer: this._getUseSRGBBuffer(!!useSRGBBuffer, noMipmap),
            },
            false,
            InternalTextureSource.Raw
        );
        internalTexture.invertY = invertY;
        internalTexture._buffer = buffer;
        internalTexture.isReady = true;
