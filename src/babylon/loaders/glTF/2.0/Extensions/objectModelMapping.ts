        ...
        extensions: {
            EXT_lights_ies: {
                multiplier: {
                    type: "number",
                    get: (node: INode) => {
                        return (node._babylonTransformNode?.getChildren((child) => child instanceof SpotLight, true)[0] as Light)?.intensity;
                    },
                    getTarget: (node: INode) => node._babylonTransformNode?.getChildren((child) => child instanceof SpotLight, true)[0] as Light,
                    set: (value, node) => {
                        if (node._babylonTransformNode) {
                            const light = node._babylonTransformNode.getChildren((child) => child instanceof SpotLight, true)[0] as Light;
                            if (light) {
                ...
                color: {
                    type: "Color3",
                    get: (node: INode) => {
                        return (node._babylonTransformNode?.getChildren((child) => child instanceof SpotLight, true)[0] as Light)?.diffuse;
                    },
                    getTarget: (node: INode) => node._babylonTransformNode?.getChildren((child) => child instanceof SpotLight, true)[0] as Light,
